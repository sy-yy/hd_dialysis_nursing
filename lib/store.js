...
function listUsers() {
  return query(usersDb, 'SELECT * FROM users').then(users => {
    return users.map(u => {
      if (u && u.permissions && typeof u.permissions === 'string') {
...
function getLogs(options = {}) {
  return new Promise((resolve) => {
    try {
      ensureLogsDir();
      const logs = [];
      const files = fs.readdirSync(LOGS_DIR).filter(f => f.endsWith('.log') && !f.startsWith('server_'));

      for (const file of files.sort()) {
        if (options.startDate || options.endDate) {
          // 支持 YYYY-MM-DD.log / access_YYYY-MM-DD.log / app_YYYY-MM-DD.log 等格式
          const dateMatch = file.match(/(\d{4}-\d{2}-\d{2})\.log$/);
          const fileDate = dateMatch ? dateMatch[1] : file.replace('.log', '');
